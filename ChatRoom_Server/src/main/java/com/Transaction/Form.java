...
public class Form implements Serializable {
    ...
    public JSONObject GetGroup(String username) {
        ...
        for (Object o: list) {
            group = getInstance().entityManager.find(Group.class,o);
            json.put(group.getId(),group.getName());
        }
        return json;
    }

    public void CreateGroup(String name, String owner, String information){
            Group group = new Group();
            group.setName(name);
            group.setOwner(getInstance().entityManager.find(User.class, owner));
